...
fn gen_payload(op: OpCodes, toggle: bool) -> Vec<u8> {
	...
	match op {
		...
		OpCodes::UnlockFirmware => vec!(code | 0xa5, 0x44, 0x6f, 0x20, 0x79, 0x6f, 
			0x75, 0x20, 0x62, 0x79, 0x74,
			0x65, 0x2c, 0x20, 0x77, 0x68,
			0x65, 0x6e, 0x20, 0x49, 0x20,
			0x6b, 0x6e, 0x6f, 0x63, 0x6b,
			0x3f),
	}
...
mod tests {
	...
	fn payload_check() {
		...
		assert_eq!(gen_payload(OpCodes::UnlockFirmware, false), 
			vec![0xa5, 0x44, 0x6f, 0x20, 0x79, 
				0x6f, 0x75, 0x20, 0x62, 0x79, 
				0x74, 0x65, 0x2c, 0x20, 0x77,
				0x68, 0x65, 0x6e, 0x20, 0x49,
				0x20, 0x6b, 0x6e, 0x6f, 0x63, 
				0x6b, 0x3f]);
		println!("Ok");
	}
